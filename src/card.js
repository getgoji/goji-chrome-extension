 ...
 */
function setupTab() {
  shadow.getElementById("goji-tab--icon").src =
    chrome.runtime.getURL("icons/goji-tab.png");
}
...
function setupBrand() {
  getData().then((data) => {
    ...
    // Apply brand name
    shadow.getElementById("goji-brand-card--name").innerHTML =
      brandData[DataCol.NAME];

    // Get HTML elements
    const categoryPercentiles = shadow.querySelectorAll(
      ".goji-brand-card--category-percentile"
    );
    const categoryNames = shadow.querySelectorAll(
      ".goji-brand-card--category-name"
    );
    ...
    preferencesSorted.forEach((preference, index) => {
      ...
      categoryNames[index].innerHTML =
        "<i>%ile</i> " + CATEGORY_NAMES[preference];
    });

    // Print Goji score
    printGojiScore(brandData[DataCol.TOTAL], "goji-brand-card--overall-score");
    printGojiScore(
      personalizedPercentileTotal / 3,
      "goji-brand-card--personalized-score"
    );
